...
OUT_BASE="${1:-$BENCH_DIR/out_cache_cheney_cache_size_$(date +%Y%m%d_%H%M%S)}"
...
for size in "${SIZES[@]}"; do
  ...
  echo "[$(date -Is)] build cache_cheney CACHE_SIZE_KB=$size"
  (
    cd "$BUILD_DIR" || exit 1
    make OPT_GC=cache_cheney CACHE_SIZE_KB="$size" -B -j2
  ...
  cp "$BUILD_DIR/ejsvm" "$size_dir/ejsvm_cache_cheney_cache_${size}"
