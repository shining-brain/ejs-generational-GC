...
OUT_BASE="${1:-$BENCH_DIR/out_giysbf_s16_fair_$(date +%Y%m%d_%H%M%S)}"
case "$OUT_BASE" in
  /*) ;;
...
TARGET_YOUNG_AFTER_AUX_KB_VALUE=524.24

GIY_GC_STACK_BYTES_VALUE=49152
GIY_LOCAL_PADDING_BYTES_VALUE=81920
CHENEY_LOCAL_PADDING_BYTES_VALUE=131072

GIY_NT_COPY_BITS_VALUE=256
GIY_OLD_SLOT_NT_STORE_VALUE=0
GIYSB_STAGING_BYTES_VALUE=16384
GIYSB_TINY_TABLE_BYTES_VALUE=65536

CONFIGS=(
  giysbf512_s16_table64_896
  giysbf256_s16_table64_896
  giy_young524_896
  cheney_young524_896
)
...
build_config() {
  ...
  if [[ "$config" == "giysbf512_s16_table64_896" ]]; then
    (
      cd "$BUILD_DIR" || exit 1
      make OPT_GC=giy \
  ...
  elif [[ "$config" == "giysbf256_s16_table64_896" ]]; then
    (
      cd "$BUILD_DIR" || exit 1
      make OPT_GC=giy \
        CACHE_SIZE_KB="$CACHE_SIZE_KB_VALUE" \
        ...
        GIYSB_STAGING_BYTES="$GIYSB_STAGING_BYTES_VALUE" \
        GIYSB_TINY_TABLE_BYTES="$GIYSB_TINY_TABLE_BYTES_VALUE" \
        GIYSB_TINY_OBJECT_MAX_BYTES=256 \
        -B -j2
    ) > "$config_dir/build.log" 2>&1
  elif [[ "$config" == "giy_young524_896" ]]; then
    (
      cd "$BUILD_DIR" || exit 1
      make OPT_GC=giy \
        CACHE_SIZE_KB="$CACHE_SIZE_KB_VALUE" \
  ...
  elif [[ "$config" == "cheney_young524_896" ]]; then
    (
      cd "$BUILD_DIR" || exit 1
